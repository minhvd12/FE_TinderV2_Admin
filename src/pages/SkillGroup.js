...
// sections
import { SkillGroupTableToolbar, SkillGroupTableRow } from '../sections/skillGroup';
  ...
  const handleDeleteRow = (id) => {
    axios({
      ...
      .catch((error) => {
        ...
        setOpenAlert(true);
        setSeverity('error');
        setMessageAlert('Nhóm kỹ năng này không tồn tại');
        } else {
          console.log(error);
          setOpenAlert(true);
          setSeverity('error');
          setMessageAlert('Nhóm kỹ năng này đang được sử dụng không thể xoá');
        }
      });
  };
  const onSubmit = () => {
  ...
  return (
    <Page title="Nhóm kỹ năng">
      <Container maxWidth={themeStretch ? false : 'lg'}>
        <HeaderBreadcrumbs
          ...
          links={[
            ...
            { name: 'Kỹ năng', href: PATH_DASHBOARD.management.skillGroup },
            { name: 'Nhóm kỹ năng' },
          ]}
          action={
            <Button
        ...
        <Card>
          ...
          <SkillGroupTableToolbar filterName={filterName} onFilterName={handleFilterName} />
          {loadingData ? (
      ...
      <Dialog
        ...
        maxWidth="xs"
      >
        <DialogTitle id="alert-dialog-title">Tạo nhóm kỹ năng</DialogTitle>
        <DialogContent>
        ...
        <DialogActions>
          ...
          <Button variant="contained" color="primary" disabled={names.length === 0} onClick={onSubmit}>Tạo</Button>
        </DialogActions>
      </Dialog>
      <Snackbar
        open={openAlert}
